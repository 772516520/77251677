...
export type Apk = typeof Android;
export type Device = ReturnType<typeof GenerateDevice.generateFullDevice>
export type device = ReturnType<typeof GenerateDevice.generateShortDevice>
...
export function getApklist(p: Platform) {
    return apklist[p];
}

export default class GenerateDevice {
    /** 生成设备信息 */
    public static generateShortDevice(uin: number) {
        const hash = Utils.md5(String(uin));
    ...
    public static generateFullDevice(d: device | number) {
        if (typeof d === "number")
        ...
        return {
            ...
            imsi: randomBytes(16),
            guid: Utils.md5(Buffer.concat([Buffer.from(d.imei), Buffer.from(d.mac_address)])),
        };
    }

    public static generateImei(uin: number) {
        let imei = uin % 2 ? "52" : "77";
        const buf = Buffer.alloc(4);
        buf.writeUint32BE(uin);
        let a: string | number = buf.readUInt16BE();
        let b: string | number = Buffer.concat([Buffer.alloc(1), buf.slice(1)]).readUInt32BE();
        if (a > 9999)
            a = Math.trunc(a / 10);
        else if (a < 1000)
